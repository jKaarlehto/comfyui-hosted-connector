...
Connector 1.1.1 reports progress to the invitation tab and shows **Saved workspaces** in its native window. Both reusable and one-time invitations are saved automatically. Existing reusable invitations in the connector's `.env` are imported locally when it opens, without starting a server. Select a saved workspace and click Connect without finding the original link. The page shows server startup stages, model transfer and verification progress, errors and disconnection. Once connected, it provides **Open ComfyUI**, the local address and **Copy address**. Keep the connector open while working. If the browser does not open it, **Reconnect** appears after twenty seconds. Connection failures also offer Reconnect. Reloading the page requires clicking Connect again; the connector attaches the new page to an existing connection for the same invitation.

Saved reusable credentials, device credentials and SSH private keys are protected with Windows DPAPI for the current Windows account on this computer. Public workspace descriptors contain no credentials, and `.env` contains only a local bookmark. Copying the workspace files or bookmark to another computer does not grant access; request a separate invitation there. **Remove** deletes the selected workspace's local credentials, clears its saved bookmark and closes its active connection. It does not revoke access on the server. Ask the owner to revoke lost or unwanted access; request an invitation to add a removed workspace again.
...
Double-click `start_hosted_comfyui.bat`, paste the invitation and wait for the progress steps. The script stores DPAPI-protected credentials under `workspaces/` beside its `.env` file, for both one-time and reusable invitations. `HOSTED_COMFYUI_ACCESS` contains only a nonsecret bookmark; older saved reusable invitations are replaced with bookmarks after import. Later launches open the saved workspace. The script establishes an SSH tunnel bound only to loopback. The final window shows the WebUI link and the local address/port for Notch. Keep the launcher open while using the connection.
